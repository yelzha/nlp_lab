...
if [ ! -f "$ORCHESTRATOR_SCRIPT" ]; then
    ...
    echo "Please ensure 'orchestrator_single_experiment.sh' is in the same directory or provide its full path."
    exit 1
fi
# The executable check is removed as we explicitly call bash
# if [ ! -x "$ORCHESTRATOR_SCRIPT" ]; then
#     echo "Error: orchestrator_single_experiment.sh is not executable. Please run 'chmod +x $ORCHESTRATOR_SCRIPT'."
#     exit 1
# fi
