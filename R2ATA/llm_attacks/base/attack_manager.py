...
class PromptManager(object):
    ...
    @property
    def goal_str(self):
        return self._prompts[0].goal_str
    
    @property
    def goal_toks(self):
        return self._prompts[0].goal_toks


    @goal_str.setter
    def goal_str(self, goal):
        for prompt in self._prompts:
            prompt.goal_str = goal
    
    @goal_toks.setter
    def goal_toks(self, goal_toks):
        for prompt in self._prompts:
            prompt.goal_toks = goal_toks
...
class MultiPromptAttack(object):
    ...
    @property
    def goal_str(self):
        return self.prompts[0].goal_str
    
    @goal_str.setter
    def goal_str(self, goal):
        for prompts in self.prompts:
            prompts.goal_str = goal
    
    @property
    def goal_toks(self):
        return [prompts.goal_toks for prompts in self.prompts]
    
    @goal_toks.setter
    def goal_toks(self, goal):
        if len(goal) != len(self.prompts):
            raise ValueError("Must provide goal tokens for each tokenizer")
        for i in range(len(goal)):
            self.prompts[i].goal_toks = goal[i]
